    ...
    async def get_or_build_deal_payout_txn(self, deal_uid: str) -> Optional[Dict[str, Any]]:
        ...
        if deal.status in ("processing", "resolving_sender", "resolving_receiver"):
            ...
            if existing_payout and isinstance(existing_payout, dict):
                ...
                if not raw:
                    pass  # Нет payload — проверку срока не делаем
                else:
                    ref_ts = raw.get("ref_block_timestamp") or raw.get("refBlockTimestamp") or raw.get("timestamp")
                    ...
                    MIN_VALID_TS_MS = 1577836800000  # 2020-01-01 00:00:00 UTC — отсекаем 0 и битые значения
                    MIN_EXPIRATION_SPAN_MS = 10 * 60 * 1000  # 10 мин — если expiration почти равен timestamp, это не срок жизни
                    expired = False
                    if ref_ts is not None and ref_ts >= MIN_VALID_TS_MS and (now_ms - ref_ts) > 24 * 3600 * 1000:
                        expired = True
                    use_expiration = (
                        expiration is not None
                        and ref_ts is not None
                        and (expiration - ref_ts) >= MIN_EXPIRATION_SPAN_MS
                    )
                    if use_expiration and expiration >= MIN_VALID_TS_MS and expiration < now_ms:
                        expired = True
                    if expired:
                        deal.payout_txn = None
                        deal.updated_at = datetime.now(timezone.utc)
